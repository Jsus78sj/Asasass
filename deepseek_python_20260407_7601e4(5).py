...
BOT_TOKEN      = "8977087578:AAFLrLCw-Ve7N04A7IuYjbtuGVgFnEL0_mM"
OWNER_ID       = 8588392906
OTP_GROUP_ID   = -1003875034789
BOT_NAME       = "𝙁𝘼𝙑𝙄𝙆𝙄𝙉 𝒃𝒐𝒕 𝒏𝒖𝒎𝒃𝒆𝒓"
CHANNEL_LINK   = "https://t.me/eheyeh7"
BOT_LINK       = "https://t.me/Whehdhhrhrbot"
DEVELOPER      = " @S_eqa"
GROUP_LINK     = "https://t.me/ueheurhd"

REQUIRED_CHANNELS = [
    {"link": "https://t.me/eheyeh7", "name": "𝙁𝘼𝙑𝙄𝙆𝙄𝙉Channel"},
    {"link": "https://t.me/ueheurhd", "name": "𝙁𝘼𝙑𝙄𝙆𝙄𝙉 Group"},
]

TIMESMS_ACCOUNTS = [
    {"username": "SOKUNAKGG", "password": "eFe576Xn", "label": "T#1"},
    {"username": "SOKUNAKGG", "password": "eFe576Xn", "label": "T#2"},
]

IMSSMS_ACCOUNTS = [
    {"username": "SOKUNAKGG", "password": "eFe576Xn", "label": "S#1"},
    {"username": "SOKUNAKGG", "password": "eFe576Xn", "label": " S#2"},
]

TIMESMS_BASE = "http://145.239.130.45/ints"
IMSSMS_BASE  = "http://145.239.130.45/ints"
...
def is_subscribed(uid):
    ...
    # First check if user is already marked as subscribed in DB
    r = qone("SELECT subscribed FROM users WHERE user_id=?", (uid,))
    if r and r["subscribed"]:
        return True
        
    # If not in DB, we need to check via Telegram API
    chs = get_sub_channels()
    if not chs:
        return True
        
    for ch in chs:
        # Extract chat_id from link (e.g., https://t.me/channelname -> @channelname)
        chat_id = ch["channel_link"].replace("https://t.me/", "@")
        res = tg("getChatMember", chat_id=chat_id, user_id=uid)
        if not res.get("ok"):
            # If we can't check (e.g. bot not admin), we skip this channel
            continue
    ...
    # If we reached here, user is in all channels
    set_subscribed(uid)
    return True
...
def handle_callback(cb_id, uid, data, mid, username="", fname=""):
    ...
    if data == "sub_check":
        if is_subscribed(uid):
            answer_cb(cb_id, "✅ Thank you for joining!", alert=False)
            edit(uid, mid, home_text(), markup=kb_services_for_user(uid))
        else:
            answer_cb(cb_id, "❌ You haven't joined all channels yet!", alert=True)
        return
...
def handle_text(uid, text, username="", fname=""):
    ...
    if step == "wait_broadcast":
        ...
        if m:    link, name = f"https://t.me/{m.group(1)}", f"@{m.group(1)}"
